...
//
// Smoke-level: one Scrum board page that exercises JiraShell + SoftwareSidebar
// + BoardColumn + IssueCard against 8 seeded issues in Sprint 42. The point is
// to render Jira Software chrome on Vercel so the SE can screenshot it.
//
// Clicking a card selects it and opens a right-side detail pane with the full
// issue view (summary, status, priority, reporter/assignee, story points, epic
// link, description). No deep routing beyond that — this is a smoke.
//
// Fidelity anchor: PLATFORMS/jira.md § UI PATTERNS > Jira Software > Board &
// Issue view. Column color is driven by status category, not name. Epic-colored
// left border per card. WIP limit pills go red when exceeded.
...
import { seedIssues, boardColumns, epics } from "./data/issues";

export default function JiraSoftwareSmokePage() {
  const [selectedKey, setSelectedKey] = useState<string | null>(null);

  // Group issues by column id for the board.
  const columnIssues = useMemo(() => {
    const by: Record<string, typeof seedIssues> = {};
    for (const col of boardColumns) by[col.id] = [];
    for (const issue of seedIssues) by[issue.columnId]?.push(issue);
    return by;
  }, []);

  const selected = useMemo(
    () => seedIssues.find((i) => i.key === selectedKey) ?? null,
    [selectedKey]
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Projects / <span style={{ color: jiraColors.textPrimary }}>Acme Web Platform</span> / Sprint 42
          </nav>
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <div
                ...
                style={{ color: jiraColors.textSecondary }}
              >
                Apr 20 – May 3 · {seedIssues.length} issues
              </div>
            </div>
              ...
              <div className="px-4 py-4">
                ...
                >
                  ...
                  <dl className="grid grid-cols-[120px_1fr] gap-y-2 text-[13px]">
                    ...
                    <dt style={{ color: jiraColors.textSecondary }}>Epic link</dt>
                    <dd>
                      {selectedEpic ? (
                        <span
                        ...
                        <span style={{ color: jiraColors.textSecondary }}>—</span>
                      )}
                    </dd>
