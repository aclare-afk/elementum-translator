// Client-side queue view. Receives already-serialized rows from the server
// component `page.tsx`, owns the selected-row state and the right-pane detail
// rendering. Split out of page.tsx because page.tsx is now a Server Component
// that reads from _lib/store.ts, and functions / event handlers can't cross
...
import type { RequestSeed } from "./data/requests";
...
export function QueueClient({
  ...
  const queueList = useMemo(() => {
    const open = rows.filter((r) => r.status.category !== "done").length;
    const incidents = rows.filter(
      (r) => r.requestTypeId === "28" && r.status.category !== "done",
    ).length;
    const waitingApproval = rows.filter(
      (r) => r.approval?.finalDecision === "pending",
    ).length;
    return [
      { id: "all-open", name: queueName, count: open },
      ...
      >
        <div className="flex flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4">
            <QueueTable
              queueName={queueName}
              summary={`${rows.length} issues · sorted by SLA urgency`}
              columns={columns}
              rows={rows}
                ...
                {selected.approval && (
                  <div className="mb-4">
                    <ApprovalPanel
                      ...
                      onDecision={(d) =>
                        console.log(
                          `Approval decision for ${selected.key}: ${d}`,
                        )
                      }
                    />
                  </div>
